...
!!  particles at obstacle boundaries should be considered as
!!  undefined.
!!
!!***
...
#include "constants.h"
  ...
  integer :: i,j,ii,jj
  REAL, DIMENSION(1) :: indX
  REAL, DIMENSION(14) :: indY  
  

  ! Trying to create an obstacle geometry in the lower left corner of the domain
  initialDomain = .true.
  boundaries = NOT_BOUNDARY 

  indX = (/1/)
  indY = (/1,2,3,4,5,6,7,12,13,14,15,16,17,18/)
  ...
  do ii=1,size(indX)
   do jj=1,size(indY)
    ...
    ! no-slip corresponds to reflecting
    ! x- boundaries
    !boundaries(ILO_FACE,i,j,1)=REFLECTING ! no need for this as it doesn't touch the domain
    boundaries(:,i,j,1)=REFLECTING

   enddo 
  enddo

  ! y-boundaries for the lower and upper part of the electrodes
  !boundaries(JLO_FACE,i,j,1)=REFLECTING ! no need for this as it doesn't touch the domain
